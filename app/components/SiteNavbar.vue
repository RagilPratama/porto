...
<template>
  <nav :class="[
      ...
      <div class="flex items-center justify-between h-14">
        ...
        <div class="hidden md:flex items-center gap-0.5">
          <a v-for="link in navLinks"
             ...
             :class="activeSection === link ? 'text-primary dark:text-cyan-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'">
            <span v-if="activeSection === link"
              class="absolute inset-0 bg-primary/8 dark:bg-cyan-400/10 rounded-lg transition-all duration-300"></span>
            <span class="relative">{{ t(`nav.links.${link}`) }}</span>
            <span v-if="activeSection === link"
              class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-gradient-to-r from-primary to-cyan-400 rounded-full"></span>
          </a>
        </div>
        ...
        <div class="flex items-center gap-1 ml-auto">
          ...
          <button @click="switchLanguage"
            ...
            <svg v-if="locale === 'en'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 20" class="w-4 h-4 rounded-sm" aria-hidden="true">
              <rect width="30" height="10" y="0" fill="#ce1126" />
              <rect width="30" height="10" y="10" fill="#fff" />
            </svg>
            <svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 20" class="w-4 h-4 rounded-sm" aria-hidden="true">
          ...
          <button @click="toggleColorMode"
            ...
            <client-only>
              <svg v-if="colorMode.value === 'dark'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-amber-400 group-hover:rotate-90 transition-transform duration-500">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
              </svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-slate-500 group-hover:-rotate-90 transition-transform duration-500">
          ...
          <a @click.prevent="scrollTo('contact')" href="#"
          ...
          <button @click="isMobileMenuOpen = !isMobileMenuOpen"
            ...
            <div class="flex flex-col gap-1 w-4">
              <span :class="['h-0.5 bg-slate-600 dark:bg-slate-300 rounded-full transition-all duration-300 origin-center', isMobileMenuOpen ? 'rotate-45 translate-y-[3px]' : '']"></span>
              <span :class="['h-0.5 bg-slate-600 dark:bg-slate-300 rounded-full transition-all duration-300', isMobileMenuOpen ? 'opacity-0 scale-x-0' : '']"></span>
              <span :class="['h-0.5 bg-slate-600 dark:bg-slate-300 rounded-full transition-all duration-300 origin-center', isMobileMenuOpen ? '-rotate-45 -translate-y-[3px]' : '']"></span>
            </div>
          </button>
        </div>
    ...
    >
      <div v-if="isMobileMenuOpen" class="md:hidden mt-2 mx-4 p-3 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-white/30 dark:border-white/8 shadow-2xl">
        <a v-for="link in navLinks"
           :key="`m-${link}`"
          ...
          <span v-if="activeSection === link" class="w-1.5 h-1.5 rounded-full bg-primary dark:bg-cyan-400"></span>
        ...
        <div class="mt-2 pt-2 border-t border-slate-200/60 dark:border-slate-700/60">
          <a @click.prevent="scrollTo('contact'); isMobileMenuOpen = false" href="#"
