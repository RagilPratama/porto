...
<template>
  <section class="py-24 scroll-mt-40 relative overflow-hidden" id="portfolio">
    <div
      class="absolute inset-0 pointer-events-none section-texture section-texture-portfolio opacity-70"
    ></div>
    <div class="max-w-7xl mx-auto px-8 relative z-10">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
        <div data-animate="fade-right">
          <span
            class="font-label text-xs tracking-[0.3em] uppercase text-primary dark:text-primary-fixed-dim mb-3 inline-flex items-center gap-2 font-bold"
          >
            <span class="w-8 h-0.5 bg-primary inline-block"></span>
            {{ t('portfolio.featured') }}
          </span>
          <h2
            class="font-headline text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight"
          >
            {{ t('portfolio.title') }}
            <span class="text-primary font-extrabold">({{ countByCategory.all }})</span>
          </h2>
          <p class="text-slate-600 dark:text-slate-300 max-w-xl mt-3 font-medium">
        ...
        <div data-animate="fade-left" class="flex flex-wrap gap-2.5 shrink-0">
          ...
          >
            ...
            <span
              :class="[
                'text-[10px] font-extrabold px-2 py-0.5 rounded-full leading-none',
                activeCategory === cat.id
        ...
        >
          <template v-if="isWide(project)">
            <div class="md:grid md:grid-cols-2 h-full">
              <div
                class="relative h-52 md:h-full overflow-hidden flex items-center justify-center p-6 bg-white/20 dark:bg-slate-900/30"
              >
                <div :class="['absolute inset-0 dots opacity-40', themeFor(project).dots]"></div>
                <span
                  v-if="isFeatured(project)"
                  class="absolute bottom-4 left-5 font-label text-[10px] tracking-[0.25em] uppercase text-primary dark:text-primary-fixed-dim font-extrabold"
                  >{{ t('portfolio.featured') }}</span
                >
                <div
                  :class="[
                    'relative z-[1] w-40 h-40 md:w-44 md:h-44 rounded-2xl bg-white/70 dark:bg-slate-800/70 border border-white/80 dark:border-white/10 shadow-xl backdrop-blur-md transition-all duration-500 group-hover:scale-110 group-hover:-rotate-2',
                ...
                <div class="absolute top-3.5 left-4">
                  <span
                    :class="[
                      'text-[10px] font-extrabold px-3 py-1 rounded-full border border-white/30 shadow-sm',
              ...
              <div class="p-6 md:p-7 flex flex-col justify-center relative">
                <div v-if="isFeatured(project)" class="flex items-center gap-2 mb-3">
                  <span
                    class="inline-flex items-center gap-1.5 font-label text-[11px] tracking-[0.25em] uppercase text-primary dark:text-primary-fixed-dim font-extrabold"
                  >
                ...
                <h3
                  class="font-headline text-xl md:text-2xl font-extrabold mb-2 text-slate-900 dark:text-white group-hover:text-primary transition-colors tracking-tight"
                >
                  {{ project.title }}
                </h3>
                <p
                  class="text-slate-600 dark:text-slate-300 text-xs sm:text-[13px] leading-relaxed mb-4 font-medium max-w-prose"
                >
                  {{ project.description }}
                </p>
                <div class="flex flex-wrap gap-1.5 mb-5">
                  <span
                    ...
                    :key="tag"
                    :class="[
                      'text-[10px] font-bold px-2.5 py-1 rounded-lg border border-white/40 dark:border-white/10 shadow-sm',
                      themeFor(project).chip
                    ]"
                    >{{ tag }}</span
                  >
                </div>
                <div class="flex flex-wrap items-center gap-3">
                  <template v-if="project.web">
                    <a
                      ...
                      target="_blank"
                      :class="[
                        'text-xs font-bold rounded-full border border-white/60 dark:border-white/20 px-4 py-1.5 inline-flex items-center gap-1.5 transition-all shadow-sm hover:scale-105',
                        themeFor(project).link
                      ]"
                    >
                      <svg
                  ...
                  <span
                    v-if="project.info"
                    class="text-xs text-slate-500 dark:text-slate-400 italic font-medium"
                    >{{ project.info }}</span
                  >
                </div>
              </div>
          ...
          <template v-else>
            <div
              class="relative h-40 overflow-hidden flex items-center justify-center bg-white/20 dark:bg-slate-900/30"
            >
              <div :class="['absolute inset-0 dots opacity-40', themeFor(project).dots]"></div>
              <div
                class="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-white/40 dark:from-slate-900/40 to-transparent"
              ></div>
              <div
                :class="[
                  'relative z-[1] w-28 h-28 md:w-32 md:h-32 rounded-2xl bg-white/70 dark:bg-slate-800/70 border border-white/80 dark:border-white/10 shadow-lg backdrop-blur-md transition-all duration-500 group-hover:scale-110 group-hover:-rotate-2',
                  themeFor(project).logoHover
                ]"
              >
                <NuxtImg
            ...
            <div class="p-6 pt-4">
              <h3
                class="font-headline text-lg font-extrabold mb-1.5 text-slate-900 dark:text-white group-hover:text-primary transition-colors tracking-tight"
              >
                {{ project.title }}
              </h3>
              <p
                class="text-slate-600 dark:text-slate-300 text-xs sm:text-[13px] leading-relaxed mb-4 font-medium"
              >
                {{ project.description }}
              </p>
