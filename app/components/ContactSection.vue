    ...
    <div class="max-w-7xl mx-auto px-8 relative z-10">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div class="text-center lg:text-left">
          <h2 class="font-headline text-4xl sm:text-5xl font-extrabold mb-8 tracking-tight dark:text-white">{{ t('contact.title') }} <span class="text-primary">{{ t('contact.titleAccent') }}</span></h2>
          ...
          <div class="space-y-6">
            <a href="mailto:ragilpratamaaa@gmail.com" class="flex items-center gap-6 group hover:translate-x-2 transition-transform duration-300">
              ...
              <div>
                <p class="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">{{ t('contact.email') }}</p>
            ...
            <div class="flex items-center gap-6 group">
              ...
              <div>
                <p class="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">{{ t('contact.location') }}</p>
                <p class="text-lg font-bold">{{ t('contact.locationValue') }}</p>
              </div>
            </div>
          </div>
          ...
          <form @submit.prevent="handleInquiry" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2 relative">
                <label class="text-sm font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 ml-1 transition-colors">{{ t('contact.form.name') }} <span class="text-red-500">*</span></label>
                <div class="relative">
                  <input v-model="form.name" @input="validateField('name')" @blur="validateField('name')" type="text" :placeholder="t('contact.form.namePlaceholder')" :class="['w-full bg-surface dark:bg-slate-800 border-2 rounded-xl p-4 pr-12 focus:ring-2 focus:ring-primary/30 outline-none transition-all dark:text-white', errors.name ? 'border-red-400/50 ring-2 ring-red-400/10' : 'border-transparent']" required>
              ...
              <div class="space-y-2 relative">
                <label class="text-sm font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 ml-1 transition-colors">{{ t('contact.form.email') }} <span class="text-red-500">*</span></label>
                <div class="relative">
                  <input v-model="form.email" @input="validateField('email')" @blur="validateField('email')" type="email" :placeholder="t('contact.form.emailPlaceholder')" :class="['w-full bg-surface dark:bg-slate-800 border-2 rounded-xl p-4 pr-12 focus:ring-2 focus:ring-primary/30 outline-none transition-all dark:text-white', errors.email ? 'border-red-400/50 ring-2 ring-red-400/10' : 'border-transparent']" required>
            ...
            <div class="space-y-2">
              <label class="text-sm font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 ml-1 transition-colors">{{ t('contact.form.message') }} <span class="text-red-500">*</span></label>
              <div class="relative">
                <textarea v-model="form.message" @input="validateField('message')" @blur="validateField('message')" rows="5" :placeholder="t('contact.form.messagePlaceholder')" :class="['w-full bg-surface dark:bg-slate-800 border-2 rounded-xl p-4 pr-12 focus:ring-2 focus:ring-primary/30 outline-none transition-all dark:text-white', errors.message ? 'border-red-400/50 ring-2 ring-red-400/10' : 'border-transparent']" required></textarea>
