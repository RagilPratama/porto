  ...
  <footer data-animate="fade-up" class="bg-white/40 dark:bg-slate-950/40 backdrop-blur-2xl border-t border-white/50 dark:border-white/10 w-full py-12 transition-all relative">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center max-w-7xl mx-auto px-8 gap-6 text-start">
      ...
      <div class="flex flex-wrap gap-3">
        <a v-for="social in socials" :key="social.name" :href="social.url" target="_blank" class="glass-pill px-4 py-1.5 rounded-full text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-primary dark:hover:text-primary-fixed-dim hover:scale-105 transition-all shadow-sm">
          {{ social.name }}
        </a>
      </div>
    </div>
